...
로그와 트래픽 속 이상 징후를 놓치지 않고, 그 원인을 끝까지 추적하는 걸 좋아하는 주니어 보안 분석가입니다.
단순히 탐지 도구를 다루는 것을 넘어, **왜 이 이벤트가 위협으로 판단되는지**를 근거 기반으로 설명할 수 있는 분석가가 되는 것이 목표입니다.
...
- 🔭 현재 **Wazuh / pfSense 기반 홈랩**으로 실제 공격 시나리오를 재현하고 탐지 룰을 검증하는 중입니다.
...
Linux 환경의 보안 로그를 수집·분석하여 SSH 브루트 포스 공격을 탐지하고, FIM(파일 무결성 모니터링)으로 시스템 변경 사항을 추적하는 환경을 구축했습니다.

**🔥 pfSense Network Security Lab**
WAN/LAN을 분리한 가상 네트워크 세그멘테이션 환경에서 방화벽 정책을 수립하고, 트래픽을 분석하여 비정상 접근을 식별하는 실습을 진행했습니다.
